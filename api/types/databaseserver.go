...
func (s *DatabaseServerV3) GetDatabases() (databases []Database) {
	if len(s.Spec.Databases) > 0 {
		...
		// DELETE IN 9.0: Database server used to represent a single database
		// in 7.0 and before, so in case any older database agents are still
		// heartbeating back, adapt them to the current interface where server
		// has a list of databases it proxies.
