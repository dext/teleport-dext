...
func NewDatabaseV3(name string, labels map[string]string, spec DatabaseSpecV3) (*DatabaseV3, error) {
	database := &DatabaseV3{
		Metadata: Metadata{
			Name:   name,
			Labels: labels,
...
// GetCA returns the database CA certificate bytes.
func (d *DatabaseV3) GetCA() []byte {
	return d.Spec.CACert
}

// SetCA sets the database CA certificate bytes.
func (d *DatabaseV3) SetCA(bytes []byte) {
	d.Spec.CACert = bytes
}
...
func (s *DatabaseV3) CheckAndSetDefaults() error {
	s.setStaticFields()
	...
	for key := range s.Spec.DynamicLabels {
		if !IsValidLabelKey(key) {
			return trace.BadParameter("database %q invalid label key: %q", s.GetName(), key)
		}
	}
	if s.Spec.Protocol == "" {
		return trace.BadParameter("database %q protocol is empty", s.GetName())
	}
	if s.Spec.URI == "" {
		return trace.BadParameter("database %q URI is empty", s.GetName())
	}
